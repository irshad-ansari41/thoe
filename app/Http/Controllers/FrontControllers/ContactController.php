...
class ContactController extends Controller {
    ...
    public function send_contact(Request $request) {
        ...
        if (empty($id)) {
            DB::table('contacts')->insert(['name' => $request->input('name'), 'email' => $request->input('email'), 'phone' => $request->input('phone'), 'message' => $request->input('message'), 'created_at' => date('Y-m-d h:i:s')]);
        } else {
            DB::table('contacts')->where('id', $id)->update(['message' => $request->input('message'), 'updated_at' => date('Y-m-d h:i:s')]);
        }

//        $data = ['subject' => "New enquiry has been generated", 'name' => $request->name, 'email' => $request->email, 'phone' => $request->mobile_no, 'message' => $request->comment,];
