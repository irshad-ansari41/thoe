            ...
            <div id="owl-demo-header" class="owl-carousel owl-theme carousel-full-width">
                <?php foreach ($sliders as $slider) { if($slider->banner_title_ar != "" && $slider->banner_image_ar != ""){ ?>
                    <div class="item">
                        ...
                        <?php
                        $banner_Title = preg_replace('/\s+/', '', $slider->banner_title_ar);
                        if (!empty(trim($banner_Title))) {
                            ?>
                            <div class="container Smsgbox">
                                <div class="title col-lg-6  col-md-6 col-sm-8 col-xs-9  <?= $locale == 'ar' ? 'ArBox' : '' ?>">
                                    <h2 class="vsmall" style="margin-bottom:7px;">
                                        <b><?= $slider->banner_title_ar ?></b>
                                    </h2>
                                    <?= $slider->banner_short_description_ar ?>                                
                                    <?php
                                    $explore_Link = preg_replace('/\s+/', '', $slider->explore_link_ar);
                                    if ($explore_Link != "") {
                                        if ($slider->explore_button_option == "1") {
                                            $slider_explore_Link_ar = $slider->explore_link_ar;
                                            $slider_explore_Link = preg_replace('/\s+/', '', $slider->explore_link_ar);
                                            ?>
