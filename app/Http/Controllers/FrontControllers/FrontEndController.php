...
class FrontEndController extends Controller
{
    ...
    public function update(Request $request, User $user)
    {
        ...
        if ($file = $request->file('pic')) {
            ...
            if (File::exists(STORE_PATH . $folderName . $user->pic)) {
                File::delete(STORE_PATH . $folderName . $user->pic);
            }
    ...
    public function getActivate($userId, $activationCode)
    {
        ...
        if (Activation::complete($user, $activationCode)) {
            ...
            return Redirect::route('fe-login')->with('success', Lang::get('auth/message.activate.success'));
        } else {
            // Activation not found or not completed.
            $error = Lang::get('auth/message.activate.error');
            return Redirect::route('fe-login')->with('error', $error);
        }
    }
    ...
    public function postForgotPassword(Request $request)
    {
        try {
            ...
            if (!$user) {
                return Redirect::route('fe-forgot-password')->with('error', Lang::get('auth/message.account_email_not_found'));
            }

            $activation = Activation::completed($user);
            if (!$activation) {
                return Redirect::route('fe-forgot-password')->with('error', Lang::get('auth/message.account_not_activated'));
            }
            ...
            $data = array(
                ...
                'forgotPasswordUrl' => URL::route('fe-forgot-password-confirm', [$user->id, $reminder->code]),
            );
    ...
    public function postForgotPasswordConfirm(Request $request, $userId, $passwordResetCode = null)
    {
        ...
        if (!$reminder = Reminder::complete($user, $passwordResetCode, $request->get('password'))) {
            // Ooops.. something went wrong
            return Redirect::route('fe-login')->with('error', Lang::get('auth/message.forgot-password-confirm.error'));
        }

        // Password successfully reseted
        return Redirect::route('fe-login')->with('success', Lang::get('auth/message.forgot-password-confirm.success'));
    }
